...
app.post("/report-lost-item", async (req, res) => {
  const { email, itemName, description, image, city, location, featureVector } =
    req.body;
  ...
  try {
    ...
    for (const foundItem of foundItems) {
      if (
        !foundItem.featureVector ||
        foundItem.featureVector.length !== featureVector.length
      )
        continue;

      const dotProduct = featureVector.reduce(
        (acc, val, i) => acc + val * foundItem.featureVector[i],
        0
      );
      const normA = Math.sqrt(
        featureVector.reduce((acc, val) => acc + val * val, 0)
      );
      const normB = Math.sqrt(
        foundItem.featureVector.reduce((acc, val) => acc + val * val, 0)
      );
    ...
    res
      .status(500)
      .json({ success: false, message: "Error reporting lost item" });
  }
});
app.post("/lost-item", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error adding lost item", error: err });
  }
});
...
app.get("/found-item/search", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Error searching found items",
      error: err,
    });
  }
});
...
app.delete("/remove-item/:itemId", async (req, res) => {
  ...
  try {
    ...
    if (lostItem) {
      ...
      res
        .status(200)
        .json({ message: "Lost item removed successfully", lostItem });
    } else {
      ...
      if (foundItem) {
        ...
        res
          .status(200)
          .json({ message: "Found item removed successfully", foundItem });
      } else {
...
app.get("/lost-item/search", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Error searching lost items",
      error: err,
    });
  }
});
