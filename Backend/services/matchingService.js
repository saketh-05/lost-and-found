...
const matchLostItem = async (lostItem) => {
  ...
  const lostImageFeatures = extractImageFeatures(
    path.join(__dirname, "..", "uploads", lostItem.image)
  );
  ...
  for (const foundItem of foundItems) {
    ...
    const foundImageFeatures = extractImageFeatures(
      path.join(__dirname, "..", "uploads", foundItem.image)
    );

    // Simple vector comparison (number of common features as proxy)
    const commonFeatures = lostImageFeatures.filter((feature) =>
      foundImageFeatures.some(
        (f) => JSON.stringify(f) === JSON.stringify(feature)
      )
    );
